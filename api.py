...
def initialise_plugins():
    ...
    _importing_commands = False
    if _config:
        importlib.import_module('tagger.plugins.{}'.format(_config))
...
class tests:
    ...
    def is_valid_child_index(num, true_index=False):
        ...
        if isinstance(num, (structure.Node, structure.Root)):
